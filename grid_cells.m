...
F.nSF=length(F.SF);             % MK. Number of grid cells
F.GC={};                        % MK. Grid code
F.GRID=zeros(nxy,nxy);          % MK. Grid cell activations, the size is equivalent to the world size
F.nlevel=2;
...
subplot(1,F.nSF+1,F.nSF+1);
plot(sort(diff(sort(F.grid))));
axis tight;  
a=axis; 
axis([a(1:2) 0 10]);
end
